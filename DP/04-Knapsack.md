...
### **Memoization**
  Let S denote total positive sum of array elements. Since we can assign minus(-) sign to all the elements, therefore minimum sum possible becomes -S. So the `range is -S to +S, that is 2*S+1 possible currentSum.`
  ...
  > Eg. Let S = 5, so 2S+1= 11 is our total columns. If currentSum= -5, we use -5+S=0th column to denote it. If currentSum = -1, we use -1+S=4th column; if currentSum = 5, we use 10th column. 
